...
function ageLabel(value: string): string {
  ...
  const days = Math.max(
    0,
    Math.floor((Date.now() - opened.getTime()) / 86_400_000),
  );
...
export function ExceptionCard({
  ...
  return (
    <article
      ...
      aria-label={`${item.type} exception for ${item.workerName}`}
    >
      ...
      <dl className={styles.facts}>
        ...
        <div>
          <dt>Work date</dt>
          <dd>
            {dateLabel(item.startsOn)}
            {item.endsOn && item.endsOn !== item.startsOn
              ? ` – ${dateLabel(item.endsOn)}`
              : ""}
          </dd>
        </div>
        {item.siteName && (
        ...
        <div>
          <dt>Age</dt>
          <dd>{ageLabel(item.createdAt)}</dd>
        </div>
      </dl>

      {item.cancelledAfterTravelAuthorised && (
        <p className={styles.travelWarning} role="note">
          Cancelled after travel was authorised
        </p>
      )}
      {item.travelAuthorisedAt && (
        <p className={styles.meta}>
          Travel authorised {dateLabel(item.travelAuthorisedAt)}
        </p>
      )}
      <p className={styles.summary}>{item.summary}</p>
      {!active && (
        <p className={styles.nextAction}>
          <strong>Resolution:</strong> {item.resolvedOutcome ?? "Resolved"}
          {item.resolutionReason ? ` · ${item.resolutionReason}` : ""}
          {item.resolvedAt ? ` · ${dateLabel(item.resolvedAt)}` : ""}
          {item.resolvedBy ? ` · actor ${item.resolvedBy}` : ""}
        </p>
      )}
      {item.nextAction && (
        <p className={styles.nextAction}>
      ...
      >
        ...
        {item.evidence.length ? (
          ...
          <p className={styles.meta}>No linked evidence in the projection.</p>
        )}
      </section>
        ...
        {item.claims.length ? (
          <ul>
            {item.claims.map((claim) => (
              <li key={claim.id}>
                <span className={styles.claimRole}>
                  {claim.assertedRole ?? "participant"}
                </span>
                <strong>
                  {claim.assertedByName ??
                    (claim.assertedRole === "worker"
                      ? item.workerName
                      : claim.assertedRole === "hirer"
                        ? item.hirerName
                        : claim.assertedBy) ??
                    "Unknown person"}
                </strong>
                ...
                <small>
                  {claim.stance}
                  {claim.sourceChannel
                    ? ` · Source: ${claim.sourceChannel}`
                    : ""}
                  {claim.sourceReference
                    ? ` · Reference: ${claim.sourceReference}`
                    : ""}
                  {claim.recordedBy ? ` · recorded by ${claim.recordedBy}` : ""}
                </small>
                {claim.evidenceRefs.length > 0 && (
                  <small>Evidence refs: {claim.evidenceRefs.join(", ")}</small>
                )}
              </li>
            ))}
          </ul>
        ) : (
          <p className={styles.meta}>No claims recorded yet.</p>
        )}
      </section>
        ...
        <>
          <details className={styles.disclosure}>
            ...
            <form action={claimAction} className={styles.form}>
              ...
              <label>
                Who asserted this?
                <select name="assertedRole" defaultValue="operator">
                  <option value="operator">Operator note</option>
                  {item.workerId && (
                    <option value="worker">Worker · {item.workerName}</option>
                  )}
                  {item.hirerId && (
                    <option value="hirer">Hirer · {item.hirerName}</option>
                  )}
                </select>
              </label>
              <label>
                Participant (if applicable)
                <select name="assertedBy" defaultValue="">
                  <option value="">Operator’s own note</option>
                  {item.workerId && (
              ...
              <label>
                Source
                <select name="source" defaultValue="operator_ui">
                  <option value="operator_ui">Operator workspace</option>
            ...
            <details className={styles.disclosure}>
              ...
              <form action={resolveAction} className={styles.form}>
                ...
                <button type="submit" disabled={resolvePending}>
                  {resolvePending ? "Saving…" : "Save resolution"}
                </button>
              </form>
            </details>
          )}
