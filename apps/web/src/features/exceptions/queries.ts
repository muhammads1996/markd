...
function parseState(value: unknown): ExceptionState {
  return value === "under_review" || value === "resolved" ? value : "open";
}
...
function mapClaim(value: unknown, index: number): ExceptionClaim | null {
  ...
  return {
    id,
    stance:
      firstString(assertion, "stance", "state", "value") ??
      firstString(value, "stance", "claim_stance") ??
      "asserted",
    summary: claimSummary(
      value.statement ?? value.summary ?? value.value ?? value,
    ),
...
function latestClaimFromRow(row: Row): ExceptionClaim | null {
  ...
  return {
    ...
    evidenceRefs: Array.isArray(row.latest_claim_evidence_refs)
      ? row.latest_claim_evidence_refs.filter(
          (ref): ref is string => typeof ref === "string",
        )
      : [],
...
function mapEvidence(
  ...
  return {
    ...
    kind,
    label:
      firstString(value, "label", "type", "provenance") ??
      (kind === "workmark" ? "Workmark" : "Stamp"),
    detail:
      firstString(value, "detail", "summary", "outcome") ??
      (structuredDetail || firstString(value, "source")),
  };
}
...
function mapQueueRow(row: Row): ExceptionQueueItem {
  ...
  if (latestClaim && !claims.some((claim) => claim.id === latestClaim.id))
    claims.push(latestClaim);
  const evidence = [
  ...
  if (
    workmarkId &&
    !evidence.some((item) => item.kind === "workmark" && item.id === workmarkId)
  ) {
    evidence.unshift({
      id: workmarkId,
      kind: "workmark",
  ...
  if (
    stampId &&
    !evidence.some((item) => item.kind === "stamp" && item.id === stampId)
  ) {
    evidence.push({
      id: stampId,
      kind: "stamp",
      ...
      detail: firstString(
        ...
        "stamp_summary",
        "stamp_source",
        "latest_stamp_attendance",
        "latest_stamp_completion",
        "latest_stamp_payment",
        "latest_stamp_note",
      ),
    });
  }
  ...
  const assignmentWorkmarkState = firstString(
    row,
    "workmark_evidence_state",
    "assignment_workmark_evidence_state",
  );
  if (assignmentId && !workmarkId && assignmentWorkmarkState) {
    evidence.push({
      id: `${assignmentId}-workmark-evidence`,
      kind: "workmark",
      label: "Assignment Workmark evidence",
      detail: assignmentWorkmarkState,
    });
  }
  const stampCountValue = row.stamp_count ?? row.assignment_stamp_count;
  const stampCount =
    typeof stampCountValue === "number"
      ? stampCountValue
      : Number(stampCountValue);
  if (
    assignmentId &&
    !stampId &&
    Number.isFinite(stampCount) &&
    stampCount > 0 &&
    !evidence.some((item) => item.kind === "stamp")
  ) {
    evidence.push({
      id: `${assignmentId}-stamp-evidence`,
      kind: "stamp",
      label: "Assignment Stamps",
      detail: `${stampCount} Stamp${stampCount === 1 ? "" : "s"} recorded`,
    });
  }
  return {
    ...
    type: (firstString(row, "type", "category", "exception_type") ??
      "verification_trust_concern") as ExceptionType,
    summary:
      firstString(row, "summary", "description", "reason") ??
      "No summary recorded.",
    createdAt: firstString(row, "created_at", "opened_at") ?? "",
    updatedAt:
      firstString(row, "updated_at") ??
      firstString(row, "created_at", "opened_at") ??
      "",
    version:
    ...
    cancelledAfterTravelAuthorised: booleanValue(
      row.cancelled_after_travel_authorised,
    ),
    nextAction:
      firstString(row, "next_action", "recommended_next_action") ??
      (parseState(row.state ?? row.status) === "open"
        ...
        : parseState(row.state ?? row.status) === "under_review"
          ...
          : null),
    resolvedOutcome: firstString(
      row,
      "resolution_outcome",
      "resolved_outcome",
      "outcome",
    ),
    resolutionReason: firstString(
      row,
      "resolution_reason",
      "resolution",
      "reason",
...
export function mapExceptionRows(rows: unknown): ExceptionQueueItem[] {
  if (!Array.isArray(rows))
    throw new Error("Unable to load the Exceptions queue.");
  ...
  for (const value of rows) {
    ...
    existing.claims = [
      ...existing.claims,
      ...item.claims.filter(
        (claim) => !existing.claims.some((current) => current.id === claim.id),
      ),
    ];
    existing.evidence = [
      ...existing.evidence,
      ...item.evidence.filter(
        (entry) =>
          !existing.evidence.some((current) => current.id === entry.id),
      ),
    ];
  }
...
export async function listExceptions(
  ...
  const assignmentIds = items.flatMap((item) =>
  ...
  const [workmarks, stamps] = await Promise.all([
    supabase
      .from("workmarks")
      .select(
        "id, assignment_id, evidence_state, attendance, completion, payment, source",
      )
    ...
    supabase
      ...
      .select(
        "id, assignment_id, attendance, completion, payment, note, source",
      )
      .in("assignment_id", assignmentIds),
  ]);
  ...
  for (const item of items)
    if (item.assignmentId) byAssignment.set(item.assignmentId, item);
  for (const row of workmarks.error ? [] : (workmarks.data ?? [])) {
    ...
    if (
      item &&
      evidence &&
      !item.evidence.some((entry) => entry.id === evidence.id)
    )
...
export async function listAssignmentOptions(): Promise<AssignmentOption[]> {
  ...
  const workerIds = rows.flatMap((row) =>
    typeof row.worker_id === "string" ? [row.worker_id] : [],
  );
  const organisationIds = rows.flatMap((row) =>
    typeof row.organisation_id === "string" ? [row.organisation_id] : [],
  );
  const [workers, organisations, contacts] = await Promise.all([
    ...
    organisationIds.length
      ? supabase
          ...
          .select("organisation_id, person_id, archived_at")
  ...
  const workerNames = new Map(
    (workers.data ?? []).map((row) => [
      ...
      firstString(row, "preferred_name", "display_name") ?? "Unknown worker",
    ]),
  );
  const organisationNames = new Map(
    (organisations.data ?? []).map((row) => [
      String(row.id),
      firstString(row, "display_name") ?? "Unknown hirer",
    ]),
  );
  const hirerIds = new Map(
    (contacts.data ?? []).map((row) => [
      String(row.organisation_id),
      firstString(row, "person_id"),
    ]),
  );
  return rows.map((row) => {
    ...
    const hirer =
      organisationNames.get(String(row.organisation_id)) ?? "Unknown hirer";
    ...
    return {
      ...
      hirerId: hirerIds.get(String(row.organisation_id)) ?? null,
      label: `${worker} · ${hirer} · ${startsOn} · ${id.slice(0, 8)}`,
    };
  });
}
