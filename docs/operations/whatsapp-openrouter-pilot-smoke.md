# FLO-129 provider smoke runbook
...
1. Start local Supabase and generate `apps/web/.env.local` with `pnpm env:local`
   if it does not already exist.
2. Add the blank server-only values from `.env.example` to the ignored
   `apps/web/.env.local` file. Preserve the generated `NEXT_PUBLIC_` values.
   The comments beneath each variable identify its dashboard location.
...
4. Create an OpenRouter key. Its presence enables the server-only provider;
   the approved intent/transcription model routes and conservative caps are
   centralised in `@markd/language`.
5. Start the application and expose the local webhook through an HTTPS tunnel:
   `corepack pnpm dev`. Configure Meta's callback as
   `https://<tunnel-host>/api/whatsapp/webhook`, subscription field `messages`.
...
2. Send a real text message from the test sender. Confirm one `channel_events`
   row with its provider message ID, then call the internal processor with the
   `Authorization: Bearer <SUPABASE_SERVICE_ROLE_KEY>` header. With OpenRouter
...
3. Send one real WhatsApp voice note. Confirm its `channel_media_assets` row
   retains the provider media ID; invoke the processor and verify it stores the
...
5. Invoke `POST /api/whatsapp/deliveries` with the
   `Authorization: Bearer <SUPABASE_SERVICE_ROLE_KEY>` header and a unique test
   JSON payload containing `recipientPhoneNumber`, `body`,
   `messageKind`, and `idempotencyKey`. Confirm `channel_deliveries` contains
   Meta's provider message ID and `sent` state. Observe Meta status callbacks
...
8. After a real test voice note is available, run
   `corepack pnpm test:providers -- --confirm-live --recipient <E.164-number> --voice-media-id <Meta-media-ID>`.
