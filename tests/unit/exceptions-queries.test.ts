...
describe("Exceptions queue mapping", () => {
  it("keeps participant claims and Workmark/Stamp evidence distinct", () => {
    const item = mapExceptionRows([
      ...
      version: 3,
        ...
        claims: [
          { id: "c-worker", assertion: "asserted", statement: "I waited at the pickup.", asserted_by: "10000000-0000-4000-8000-000000000001", asserted_by_name: "Amina", asserted_role: "worker", source: "whatsapp", source_reference: "wa-msg-1", evidence_refs: ["stamp:s-1"] },
          { id: "c-hirer", assertion: "disputed", statement: "The worker did not arrive.", asserted_by: "20000000-0000-4000-8000-000000000001", asserted_by_name: "Build Co", asserted_role: "hirer", source: "operator_ui" },
        ],
        workmark_evidence: [{ id: "w-1", label: "Workmark", detail: "attendance: no_show" }],
        stamp_evidence: [{ id: "s-1", label: "Stamp", detail: "operator recorded" }],
      },
    ])[0]!;
    expect(item).toMatchObject({ id: "e-1", workerName: "Amina", cancelledAfterTravelAuthorised: true });
    expect(item.claims.map(({ assertedRole }) => assertedRole)).toEqual(["worker", "hirer"]);
    expect(item.claims[0]).toMatchObject({ sourceReference: "wa-msg-1", evidenceRefs: ["stamp:s-1"] });
    expect(item.evidence.map(({ kind }) => kind)).toEqual(["workmark", "stamp"]);
  });

  it("deduplicates flattened view rows while retaining claims", () => {
    const item = mapExceptionRows([
      { exception_id: "e-1", state: "open", exception_type: "payment_dispute", statement: "Payment pending", worker_display_name: "Amina", claims: [{ id: "c-1", statement: "Pending", asserted_role: "worker" }] },
      { exception_id: "e-1", state: "open", exception_type: "payment_dispute", statement: "Payment pending", worker_display_name: "Amina", claims: [{ id: "c-2", statement: "Paid", asserted_role: "hirer" }] },
    ])[0]!;
  ...
  it("maps the compact queue projection's latest claim and Stamp evidence", () => {
    ...
    expect(item.claims[0]).toMatchObject({ assertedRole: "hirer", summary: "Only half the wall was complete." });
    expect(item.claims[0]).toMatchObject({ sourceReference: "call-42", evidenceRefs: ["workmark:w-2"] });
    expect(item.evidence).toContainEqual({ id: "s-2", kind: "stamp", label: "Stamp", detail: "partial" });
  });
  ...
  it("keeps the hirer claimant fallback and resolution outcome from the queue", () => {
    ...
    expect(item).toMatchObject({ version: 7, resolvedOutcome: "worker_confirmed", resolutionReason: "The hirer confirmed the worker attended." });
  });
  ...
  it("retains assignment-level evidence when the exception has no direct Workmark", () => {
    ...
    expect(item.evidence).toEqual([
      { id: "a-4-workmark-evidence", kind: "workmark", label: "Assignment Workmark evidence", detail: "pending" },
      { id: "a-4-stamp-evidence", kind: "stamp", label: "Assignment Stamps", detail: "2 Stamps recorded" },
    ]);
  });
